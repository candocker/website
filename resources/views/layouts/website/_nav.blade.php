...
<!--<nav class="uix-menu__container uix-trans">-->
    <div class="uix-menu__inner">
        ...
        <ul class="uix-menu"> 
            ...
            <li class="@if (isset($navData['subNavs'])) multi-column @endif @if ($navCode == $datas['bigNav']) current-menu-item is-active @endif"><a href="/{{$navCode}}">{{$navData['name']}}</a>
                @if (isset($navData['subNavs']))
                <ul class="sub-menu">
                    <li>
                        <ul class="sub-menu">
                    @foreach ($navData['subNavs'] as $subCode => $subData)
                            <li><a href="/{{$navCode}}-{{$subCode}}" style="@if ($subCode == $datas['currentNav']) color:blue; @endif">{{$subData['name']}}</a></li> @endforeach
                        </ul>
                    </li>
                </ul>
                @endif
