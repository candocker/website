    ...
    <div class="row">
      <div class="col-12">
        <div class="uix-table uix-table--bordered">
          <div class="col-12">
            <h4 class="uix-heading--pinline" style="font-size: 16px; color: lightblue;">{{$data['title']}}</h4>
            @if (isset($data['description']))<p class="uix-heading--pinline" style="color: green;font-size:14px;">{{$data['description']}}</p>@endif
          </div>
          <table>
            <tbody class="uix-t-l--md">
              @php $i = 1; @endphp
              @foreach ($data['infos'] as $bCode => $pInfo)
