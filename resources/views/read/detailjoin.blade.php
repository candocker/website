    ...
    <div class="uix-entry__box uix-entry__box--top uix-entry__box--mark uix-spacing--no">
      ...
      <div class="row">
        <div class="col-4 uix-t-l" style="padding-right:0px;font-size:12px;">
          <a class="" @if ($datas['pre']['code'])href="/readshow-{{$datas['sort']}}-{{$datas['bookCode']}}-{{$datas['pre']['code']}}"@endif>&larr; {{$datas['pre']['name']}}</a>
        </div>
        <div class="col-4 uix-t-c" style="padding-right:0px; padding-left:0px; font-size:12px;">
 　 <a href="/readlist-{{$datas['sort']}}-{{$datas['bookCode']}}">{{$datas['bookData']['name']}}</a> @if (isset($datas['bookData']['author']))<span class="foot_key">作者:</span>{{$datas['bookData']['author']}} @endif
        </div>
        <div class="col-4 uix-t-r" style="padding-left:0px; font-size:12px;">
          <a class="" @if ($datas['next']['code'])href="/readshow-{{$datas['sort']}}-{{$datas['bookCode']}}-{{$datas['next']['code']}}"@endif>{{$datas['next']['name']}} &rarr;</a>
        </div>             
      </div>
